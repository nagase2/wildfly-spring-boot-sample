...
//w = window.parent.screen.width, h = window.parent.screen.height, 
w = 900, h = 500,
margin = 30, startYear = 1960, endYear = 2010, startAge = 20, endAge = 90, y = d3.scale
		.linear().domain([ endAge, startAge ])
		.range([ 0 + margin, h - margin ]), x = d3.scale.linear().domain([
		                                                                  new Date(2010, 0, 1), 
		                                                                  new Date(2014, 12, 1)
		                                                                 ]
		).range([ 0 + margin - 5, w ]), years = d3.range(
		startYear, endYear);
...
//all...１万件データ。　Macで検証したところ、読み込み時間は５秒程度
//var csvfile='life-expectancy-cleaned-all.csv';
var csvfile='life-expectancy-cleaned-small.csv';

d3.text(csvfile, 'text/csv', function(text) {
